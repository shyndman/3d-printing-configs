class PerPrintPressureAdvance:
    def __init__(self, config):
        ...
        gcode.register_command("SET_PRINT_PRESSURE_ADVANCE",
                                   self.cmd_SET_PRINT_PRESSURE_ADVANCE,
                                   desc=self.cmd_SET_PRINT_PRESSURE_ADVANCE_help)

    cmd_SET_PRINT_PRESSURE_ADVANCE_help = "Set pressure advance automatically, or based on provided nozzle sizes"
    def cmd_SET_PRINT_PRESSURE_ADVANCE(self, gcmd):
        nozzle_size = gcmd.get('NOZZLE', parser=str, default=None)
        if nozzle_size == None:
            gcmd.respond_info("SET_PRINT_PRESSURE_ADVANCE called, but no NOZZLE argument provided")
            return
